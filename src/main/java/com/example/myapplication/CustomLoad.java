...
public class CustomLoad implements LoadControl {
    ...
    public static final class Builder {
         ...
         */
        public CustomLoad.Builder setAllocator(DefaultAllocator allocator) {
         ...
         * @throws IllegalStateException If {@link #build()} has already been called.
         */
        public CustomLoad.Builder setBufferDurationsMs(
                int minBufferMs,
                int maxBufferMs,
         ...
         * @throws IllegalStateException If {@link #build()} has already been called.
         */
        public CustomLoad.Builder setTargetBufferBytes(int targetBufferBytes) {
            checkState(!buildCalled);
         ...
         * @throws IllegalStateException If {@link #build()} has already been called.
         */
        public CustomLoad.Builder setPrioritizeTimeOverSizeThresholds(boolean prioritizeTimeOverSizeThresholds) {
            checkState(!buildCalled);
         ...
         * @throws IllegalStateException If {@link #build()} has already been called.
         */
        public CustomLoad.Builder setBackBuffer(int backBufferDurationMs, boolean retainBackBufferFromKeyframe) {
            checkState(!buildCalled);
         ...
         */
        @Deprecated
        public CustomLoad createCustomLoad() {
            return build();
        }

        /**
         * Creates a {@link CustomLoad}.
         */
        public CustomLoad build() {
            checkState(!buildCalled);
            buildCalled = true;
            if (allocator == null) {
                allocator = new DefaultAllocator(/* trimOnReset= */ true, C.DEFAULT_BUFFER_SEGMENT_SIZE);
            }
            return new CustomLoad(
                    allocator,
                    minBufferMs,
                    maxBufferMs,
                    bufferForPlaybackMs,
    ...
    public boolean shouldContinueLoading(
        ...
        compute(bufferedDurationUs);
        return isLoading;
    }

    private void compute(long bufferedDurationUs) {
        int abc = getBufferTimeState(bufferedDurationUs);
        boolean aaa = allocator.getTotalBytesAllocated() >= targetBufferBytes;
        if (abc == BELOW_LOW_WATERMARK)
            isLoading = true;
        else if (abc == BETWEEN_WATERMARKS)
            isLoading = !aaa;
        else
            isLoading = false;
    }
